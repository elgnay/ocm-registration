...
const (
	defaultComponentNamespace = "open-cluster-management-hub"
)
...
func RunControllerManager(ctx context.Context, controllerContext *controllercmd.ControllerContext) error {
	...
	namespacedKubeInformers := kubeinformers.NewSharedInformerFactoryWithOptions(
		kubeClient,
		10*time.Minute,
		kubeinformers.WithNamespace(getComponentNamespace()))
	...
	autoBindingController := managedclustersetbinding.NewAutoBindingController(
		clusterClient,
	...
	go namespacedKubeInformers.Start(ctx.Done())
